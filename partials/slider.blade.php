   ...
   <div class="carousel-inner" style="background-color: white">
     ...
     @foreach (slideshow() as $val)
		<div class="item {{$val->idSlide==1 ? 'active':''}} animated fadeInRight">
       			<img src="{{URL::to(slide_image_url($val))}}" alt="{{$val->gambar}}" class="img-responsive" style="text-align: center; margin:0 auto;" />
 ...
 <div class="container">
    <div class="row">
       <div class="col-md-12">
          ...
          <a target="_blank" href="{{URL::to($item->url)}}"><img class="img-responsive" src="{{URL::to(banner_image_url($item))}}"  /></a>
          @endforeach
          </center>
       </div>
    </div>
